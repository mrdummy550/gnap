...
Workflows describe HOW work is done — the steps, criteria, inputs, and outputs
that an agent follows when executing a class of tasks. A Workflow is to a Task
what a class is to an object: the template from which concrete instances are
created.

**Directory:** `.gnap/workflows/`

**File:** `.gnap/workflows/{workflow-id}.md`
...
#### 2.8.1 Frontmatter Fields

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `id` | string | MUST | Unique workflow identifier |
| `name` | string | MUST | Human-readable name |
| `owner` | string | MUST | Default agent ID who executes this workflow |
| `trigger` | enum | MUST | `manual`, `schedule`, `event` |
| `schedule` | string | MAY | Cron expression when `trigger=schedule` |
| `event` | string | MAY | Event name when `trigger=event` |
| `goal` | string | MUST | Goal ID from `company.json` this workflow serves |
| `tags` | array | SHOULD | Category tags |
| `inputs` | array | SHOULD | Required input parameters with types |
| `outputs` | array | SHOULD | Expected output parameters with types |
| `creates_task` | boolean | MAY | Whether execution creates a task in `.gnap/tasks/` |
| `task_template` | object | MAY | Template for auto-generated task (id pattern, tag, column) |

#### 2.8.2 Workflow vs Task vs Skill

| | Workflow | Task | Skill |
|---|---|---|---|
| **Nature** | Business process template | Concrete work instance | Technical capability |
| **Format** | Markdown (`.md`) | JSON (`.json`) | Markdown + scripts |
| **Lifecycle** | Persists, evolves | Created → executed → done | Installed, versioned |
| **Who writes** | Humans (founders) | Agents + humans | Developers |
| **Reuse** | Many times | Once | Many times |
| **Level** | Business logic | Operational | Technical |
| **Example** | "How to qualify a lead" | "Qualify lead: Acme Corp" | "How to use GitHub API" |

The relationship is:

```
Skill teaches HOW (technically)    → "I know how to call GitHub API"
Workflow defines WHAT (business)   → "When a new lead comes in, do steps 1-5"
Task is a CONCRETE INSTANCE        → "Qualify lead: Acme Corp (score: 7, hot)"
Run is the EXECUTION TRACE          → "Started 10:00, found 8 leads, cost $0.42"
```

#### 2.8.3 How Workflows Generate Tasks

When a workflow is triggered (manually, by schedule, or by event), the assigned
agent:

1. Reads the workflow file
2. Validates that required inputs are available
3. If `creates_task: true`, creates a task in `.gnap/tasks/` using `task_template`
4. Executes the workflow steps
5. Records outputs in the task comments or run artifacts
6. Updates task state based on exit criteria

```
Trigger (cron / event / manual)
         │
         ▼
Agent reads .gnap/workflows/{id}.md
         │
         ▼
Validates inputs
         │
         ▼
Creates task from task_template (if creates_task=true)
         │
         ▼
Executes steps from workflow body
         │
         ▼
Records outputs + creates run
         │
         ▼
Updates task state (done / review / blocked)
```

Agents MUST commit with message format:
`{agent}: workflow {workflow-id} → task {task-id}`

#### 2.8.4 Trigger Types

| Trigger | Description | Implementation |
|---------|-------------|----------------|
| `manual` | Human or agent explicitly invokes | Agent reads workflow on request |
| `schedule` | Runs on cron schedule | Agent checks on heartbeat if schedule matches |
| `event` | Triggered by an external event | Agent creates task when event is detected (e.g., new lead in CRM, new message) |

#### 2.8.5 Workflow Versioning

Workflows are Markdown files in git. Version history is automatic:

- `git log .gnap/workflows/lead-qualification.md` shows all changes
- Agents SHOULD read the current version on each execution (not cache)
- Breaking changes to a workflow SHOULD be committed with message:
  `{agent}: update workflow {id} — {what changed}`
